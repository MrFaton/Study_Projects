...
/*
LinkedList - обычный двусвязный список. В нём есть порядок элементов и индексация. Если я добавил элементы в
порядке 1, 2, 3 - то в таком же порядке я их и прочитаю. В таком типе коллекций удобно добавлять и удалять элементы в
начале и конце списка. Добавление / удаление из середины списка - не очень эффективно, потому что нужно прокручивать
итератор до нужно места. А метод get(int index) - вообще не эффективен.
 */